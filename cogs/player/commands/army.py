...
def _progress(items) -> str:
    if not items:
        return "—"
    maxed = sum(1 for i in items if i.level >= i.max_level)
...
async def army(interaction: discord.Interaction, tag: str = None, user: discord.User = None):
    ...
    embed.add_field(name=f"{E_HERO} Heroes", value=_progress([h for h in player.heroes if h.is_home_base]), inline=True)
    embed.add_field(name=f"{E_TROOP} Troops", value=_progress(player.home_troops), inline=True)
    embed.add_field(name=f"{E_SPELL} Spells", value=_progress(player.spells), inline=True)
